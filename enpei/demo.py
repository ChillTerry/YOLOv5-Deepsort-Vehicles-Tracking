...
class HighwayAnalyse:
   ...
   # 添加中文
    ...
    def plot_bboxes_4(self,image, bboxes,side='left'):
        """
        绘制，展示经纬度
        """
        alpha = 0.9
        ...
        for (l, t, r, b, cls_name, track_id) in bboxes:
            ...
            scale = (r- l)/40 * 0.7
            scale_t = round((r- l)/40 * 10)

            lon,lat = self.speed_esti.pixel2lonlat(head_pos[0], head_pos[1])

            cv2.putText(image, '{}'.format(lon), (l-30,t-scale_t*3), cv2.FONT_HERSHEY_SIMPLEX,scale,color,2)
            cv2.putText(image, '{}'.format(lat), (l-30,t-scale_t), cv2.FONT_HERSHEY_SIMPLEX,scale,color,2)
    ...
    def plot_bboxes_3(self,image, bboxes,side='left'):
        """
        绘制，展示ID和尾部线条
        """
        alpha = 0.9
        ...
        for (l, t, r, b, cls_name, track_id) in bboxes:
            ...
            cv2.putText(image, '{}'.format(track_id), (l,t-10), cv2.FONT_HERSHEY_SIMPLEX,scale,color,2)
    ...
    def plot_bboxes_2(self,image, bboxes,side='left'):
        """
        绘制，展示不同车型
        """
        alpha = 0.9
        ...
        for (l, t, r, b, cls_name, track_id) in bboxes:
            ...
            # 中文标签
            overlay = self.zh_label_img_list[png_id]
            ...
            overlay_l,overlay_t = l,t-overlay_h - 2

            overlay_r,overlay_b = (overlay_l + overlay_w),(overlay_t+overlay_h)

            if overlay_t > 0 and overlay_r < 281:

                # 覆盖图层
                overlay_copy=cv2.addWeighted(image[overlay_t:overlay_b, overlay_l:overlay_r ],0,overlay,1,0)
                image[overlay_t:overlay_b, overlay_l:overlay_r ] = overlay_copy
